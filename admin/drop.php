...
$query = doquery("SELECT id,name FROM {{table}} ORDER BY id", "drops");
    $template = gettemplate("admin/paginas/drop");
	$page = parsetemplate($template,$controlrow);
    $page = $xml . $page;
    $count = 1;
    while ($row = mysql_fetch_array($query)) {
        if ($count == 1) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">".$row["id"]."</td><td style=\"background-color: #eeeeee;\">".$row["name"]."</a></td></tr>\n"; }
    }
    if (mysql_num_rows($query) == 0) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">No se encontró el item.</td></tr>\n"; }
    $page .= "</table>";
