...
# *                                                          				#
#############################################################################
$query = doquery("SELECT id,usuario,charname,password,email,regdate,verify,ip FROM {{table}} ORDER BY id", "usuarios");
   $template = gettemplate("admin/paginas/usuario");
	$page = parsetemplate($template,$controlrow);
    $page = $xml . $page;
    $count = 1;
	
    while ($row = mysql_fetch_array($query)) {
        if ($count == 1) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">".$row["id"]."</td><td style=\"background-color: #eeeeee;\">".$row["usuario"]."</td><td style=\"background-color: #eeeeee;\">".$row["charname"]."</td><td style=\"background-color: #eeeeee;\">".$row["password"]."</td><td style=\"background-color: #eeeeee;\">".$row["email"]."</td><td style=\"background-color: #eeeeee;\">".$row["regdate"]."</td><td style=\"background-color: #eeeeee;\">".$row["verify"]."</td><td style=\"background-color: #eeeeee;\">".$row["ip"]."</td></tr>\n"; 
		 }
    }
