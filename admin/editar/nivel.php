...
#############################################################################
    $id= $_POST['id'];
    $query = doquery("SELECT * FROM {{table}} WHERE id='$id' LIMIT 1", "niveles");
    $row = mysql_fetch_array($query);
	$template = gettemplate("admin/paginas/editar/nivel");
	$page = parsetemplate($template, $row, $controlrow);
    $page = $xml . $page;
admindisplay($page, "Administración- Editar nivel");
?>